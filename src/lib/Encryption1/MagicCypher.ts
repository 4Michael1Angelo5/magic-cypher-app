...
import CipherObject from "../Encryption/CipherContract";
import { calculateIndexOfVertexInArray, generateIndexedUVCoordMatrix, generateTileOriginUVCoords } from "../Encryption/ImageEnryptionHelpers";
import {     
            CipherType,
            ...
            IndexedChar,
            IndexedVertex, Matrix } from "../Encryption/CipherTypes";
...
class MagicCypher<T extends CipherType> {
    ...
    runEncryption = async (input:EncryptionInput<T>) :Promise<EncryptionOutput<T>>=>{
        ...
        try { 
            const cipherObject:CipherObject<T> = await this.determineCipher(order,childParams);
    ...
    determineCipher = async (order:number, childParams:ChildParams<T>):Promise<CipherObject<T>>=>{
        ...
        if(order%2===0){
            ...
            if(order%4===0){
                ...
                const { default: EvenCypher } = await import('../Encryption/EvenCypher');
                cipherObject = new EvenCypher<T>(childParams);   
            
            }else{
                console.log("(new class) singly even");
                const { default: SinglyEvenCypher } = await import('../Encryption/SinglyEvenCypher');
                cipherObject = new SinglyEvenCypher<T>(childParams);   
            }                                      
        }else{
            console.log("(new class) odd cipher")
            const { default: OddCypher } = await import('../Encryption/OddCypher');
    ...
    //     console.log("this should be the identity square", matrix);
    ...
    //         const cipherObject:CipherObject<T> = await this.determineCipher(this.order,childParams); 

    //         const output:EncryptionOutput<T> = cipherObject.decrypt(input.type as T);

    //         console.log("output from new class called from within class: "); 
